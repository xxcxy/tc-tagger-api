...
app.use((err, req, res, next) => {
  ...
  const errorResponse = {}
  const status = err.isJoi ? HttpStatus.StatusCodes.BAD_REQUEST : (err.status || err.httpStatus || HttpStatus.StatusCodes.INTERNAL_SERVER_ERROR)
  ...
  if (_.isUndefined(errorResponse.message)) {
    if (err.message && (err.httpStatus || status !== HttpStatus.StatusCodes.INTERNAL_SERVER_ERROR)) {
